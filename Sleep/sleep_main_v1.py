...
def get_args():
    ...
    parser.add_argument("-cn", "--cut_num", help="vcf拆分数量",type=int,default=100)
    ...
    parser.add_argument("-kl", "--kegg_lib", help="kegg 依赖库", default='')
    parser.add_argument("-gl", "--go_lib", help="go 依赖库", default='')
    parser.add_argument("-sl", "--split_log", help="拆分列表", default='split.log')
...
if __name__ == '__main__':
    ...
    with open(split_log,'w') as log_obj:
        for i in range(cut_num):
            print(f'split_{i+1}',file=log_obj)
    ...
    sub_cmd = f'''{snakemake} -s {snakemake_script} --configfile input_config.yaml --cluster-config {cluster_contig_path} --cluster "qsub -V -q all.q,big -cwd -l vf={{cluster.memory}},p={{cluster.threads}} -o {{cluster.output}} -e {{cluster.error}}" --jobs {job} --latency-wait {wait_time} --restart-times 3
    '''
    print('命令',sub_cmd)
